...
test('resolveConfig: falls back to placeholder repo and "latest" when piggy field is absent', () => {
  assert.deepEqual(lib.resolveConfig({}), { repo: 'doramirdor/piggy', version: 'latest' });
  assert.deepEqual(lib.resolveConfig(undefined), { repo: 'doramirdor/piggy', version: 'latest' });
});
...
test('buildReleaseApiUrl: "latest" hits the /releases/latest endpoint', () => {
  assert.equal(
    lib.buildReleaseApiUrl('doramirdor/piggy', 'latest'),
    'https://api.github.com/repos/doramirdor/piggy/releases/latest'
  );
  assert.equal(
    lib.buildReleaseApiUrl('doramirdor/piggy', undefined),
    'https://api.github.com/repos/doramirdor/piggy/releases/latest'
  );
});
...
test('buildReleaseApiUrl: a pinned version hits the /releases/tags/<version> endpoint', () => {
  assert.equal(
    lib.buildReleaseApiUrl('doramirdor/piggy', 'v0.1.0'),
    'https://api.github.com/repos/doramirdor/piggy/releases/tags/v0.1.0'
  );
});
...
test('buildNoReleaseMessage: friendly message points at the releases page', () => {
  const msg = lib.buildNoReleaseMessage('doramirdor/piggy');
  assert.equal(
    msg,
    "Piggy hasn't shipped its first release yet - watch https://github.com/doramirdor/piggy/releases"
  );
});
