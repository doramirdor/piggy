...
function adviceItem(c: SnapCandidate, applied: boolean): AdviceItem {
  return {
    ...
    // The fixture is generated by `piggy advise --json`, which is the CLI and
    // has no advisor in it, so a blocked trim here is blocked for the one
    // reason the CLI can be blocked for: no local model.
    draftState: c.blocked ? "unavailable" : null,
